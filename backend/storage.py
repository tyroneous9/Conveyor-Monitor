"""SQLite storage for raw accelerometer windows and their derived FFT spectra.

Writes come from one process at a time (ingest.py, then separately analyze_fft.py), write volume
is one row per window, and a single file needs no separate daemon competing
with Mosquitto + these scripts for the Pi's resources. WAL mode is enabled
so /analysis notebooks can read the file concurrently with either writer.

"""

import json
import sqlite3
import time

SCHEMA = """
CREATE TABLE IF NOT EXISTS raw_windows (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    received_at REAL NOT NULL,
    sample_rate_hz REAL NOT NULL,
    ax TEXT NOT NULL,
    ay TEXT NOT NULL,
    az TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS fft_results (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    window_id INTEGER NOT NULL REFERENCES raw_windows(id),
    analyzed_at REAL NOT NULL,
    sample_rate_hz REAL NOT NULL,
    freq_hz TEXT NOT NULL,
    fft_ax TEXT NOT NULL,
    fft_ay TEXT NOT NULL,
    fft_az TEXT NOT NULL,
    peak_axis TEXT NOT NULL,
    peak_freq_hz REAL NOT NULL,
    peak_amp REAL NOT NULL
);

CREATE TABLE IF NOT EXISTS baselines (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    feature TEXT NOT NULL,
    mean REAL NOT NULL,
    std REAL NOT NULL,
    n_windows INTEGER NOT NULL,
    computed_at REAL NOT NULL
);

CREATE TABLE IF NOT EXISTS classifications (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    window_id INTEGER NOT NULL REFERENCES raw_windows(id),
    feature_value REAL NOT NULL,
    threshold REAL NOT NULL,
    predicted_label TEXT NOT NULL,
    classified_at REAL NOT NULL
);

CREATE TABLE IF NOT EXISTS window_labels (
    window_id INTEGER PRIMARY KEY REFERENCES raw_windows(id),
    label TEXT NOT NULL,
    labeled_at REAL NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_raw_windows_time
    ON raw_windows(received_at);
CREATE INDEX IF NOT EXISTS idx_fft_results_time
    ON fft_results(analyzed_at);
CREATE INDEX IF NOT EXISTS idx_fft_results_window
    ON fft_results(window_id);
CREATE INDEX IF NOT EXISTS idx_classifications_window
    ON classifications(window_id);
"""


def connect(db_path):
    conn = sqlite3.connect(db_path)
    conn.execute("PRAGMA journal_mode=WAL")
    conn.executescript(SCHEMA)
    conn.commit()
    return conn


def store_window(conn, payload, received_at=None):
    """Write one raw accel window as-is. Returns the new row's id.

    received_at defaults to now; callers backdating a session (see
    backend/window_gen.py) can override it.
    """
    cur = conn.execute(
        "INSERT INTO raw_windows (received_at, sample_rate_hz, ax, ay, az) "
        "VALUES (?, ?, ?, ?, ?)",
        (
            received_at if received_at is not None else time.time(),
            payload["sample_rate_hz"],
            json.dumps(payload["ax"]),
            json.dumps(payload["ay"]),
            json.dumps(payload["az"]),
        ),
    )
    conn.commit()
    return cur.lastrowid


def fetch_unanalyzed_windows(conn, limit=None):
    """Raw windows that don't have a matching fft_results row yet.
    `limit=None` fetches all of them (SQLite's LIMIT -1 means unbounded)."""
    cur = conn.execute(
        "SELECT r.id, r.sample_rate_hz, r.ax, r.ay, r.az "
        "FROM raw_windows r "
        "LEFT JOIN fft_results f ON f.window_id = r.id "
        "WHERE f.id IS NULL "
        "ORDER BY r.id "
        "LIMIT ?",
        (limit if limit is not None else -1,),
    )
    return [
        {
            "window_id": row[0],
            "sample_rate_hz": row[1],
            "ax": json.loads(row[2]),
            "ay": json.loads(row[3]),
            "az": json.loads(row[4]),
        }
        for row in cur.fetchall()
    ]


def store_fft_result(conn, window_id, result, peak):
    """Persist one window's computed spectrum (per-axis FFT + shared
    frequency bins) plus its overall peak (axis, frequency, amplitude)."""
    axis, peak_freq_hz, peak_amp = peak
    conn.execute(
        "INSERT INTO fft_results "
        "(window_id, analyzed_at, sample_rate_hz, freq_hz, "
        " fft_ax, fft_ay, fft_az, peak_axis, peak_freq_hz, peak_amp) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (
            window_id,
            time.time(),
            result["sample_rate_hz"],
            json.dumps(result["freq_hz"]),
            json.dumps(result["fft_ax"]),
            json.dumps(result["fft_ay"]),
            json.dumps(result["fft_az"]),
            axis,
            peak_freq_hz,
            peak_amp,
        ),
    )
    conn.commit()


def store_baseline(conn, feature, mean, std, n_windows):
    """Persist a computed baseline as a durable, inspectable artifact --
    not a number silently recomputed inside a script every run. Callers
    (see analysis/classify_faults.py) can recompute and re-store to update
    it; each call adds a new row rather than overwriting, so history isn't
    lost."""
    conn.execute(
        "INSERT INTO baselines (feature, mean, std, n_windows, computed_at) "
        "VALUES (?, ?, ?, ?, ?)",
        (feature, mean, std, n_windows, time.time()),
    )
    conn.commit()


def fetch_latest_baseline(conn, feature):
    """Most recently computed baseline for one feature, or None if none has
    been stored yet."""
    row = conn.execute(
        "SELECT mean, std, n_windows, computed_at FROM baselines "
        "WHERE feature = ? ORDER BY computed_at DESC LIMIT 1",
        (feature,),
    ).fetchone()
    if row is None:
        return None
    return {"mean": row[0], "std": row[1], "n_windows": row[2], "computed_at": row[3]}


def store_classification(conn, window_id, feature_value, threshold, predicted_label):
    """Record one window's classifier verdict (feature_value vs. threshold
    -> predicted_label) for later review/scoring."""
    conn.execute(
        "INSERT INTO classifications "
        "(window_id, feature_value, threshold, predicted_label, classified_at) "
        "VALUES (?, ?, ?, ?, ?)",
        (window_id, feature_value, threshold, predicted_label, time.time()),
    )
    conn.commit()


def fetch_fft_results(conn):
    """All fft_results rows, ordered by window id, with each window's
    ground-truth label (healthy/worn/None -- see analysis/labels.py) from
    the already-stored spectrum, without recomputing the FFT."""
    rows = conn.execute(
        "SELECT r.id, r.received_at, f.sample_rate_hz, f.freq_hz, f.fft_ay, f.peak_freq_hz, f.peak_amp, l.label "
        "FROM raw_windows r JOIN fft_results f ON f.window_id = r.id "
        "LEFT JOIN window_labels l ON l.window_id = r.id "
        "ORDER BY r.id"
    ).fetchall()
    return [
        {
            "window_id": row[0],
            "received_at": row[1],
            "sample_rate_hz": row[2],
            "freq_hz": json.loads(row[3]),
            "fft_ay": json.loads(row[4]),
            "peak_freq_hz": row[5],
            "peak_amp": row[6],
            "label": row[7],
        }
        for row in rows
    ]


def clear_labels(conn):
    """Drop every stored window label, so a fresh run of analysis/labels.py
    reflects exactly the ranges it's given rather than a mix of old and new
    sessions."""
    conn.execute("DELETE FROM window_labels")
    conn.commit()


def label_windows(conn, ranges, label):
    """Label every raw window whose received_at falls in one of `ranges`
    with `label` ("healthy"/"worn"), overwriting any existing label for
    that window. Returns the number of windows labeled."""
    n = 0
    for start, end in ranges:
        cur = conn.execute(
            "INSERT INTO window_labels (window_id, label, labeled_at) "
            "SELECT id, ?, ? FROM raw_windows WHERE received_at BETWEEN ? AND ? "
            "ON CONFLICT(window_id) DO UPDATE SET label = excluded.label, labeled_at = excluded.labeled_at",
            (label, time.time(), start, end),
        )
        n += cur.rowcount
    conn.commit()
    return n


def fetch_window_by_label(conn, label):
    """First raw+spectrum window (all three axes) carrying `label`, or None
    if no window has that label yet."""
    row = conn.execute(
        "SELECT r.sample_rate_hz, r.ay, f.freq_hz, f.fft_ax, f.fft_ay, f.fft_az, "
        "f.peak_axis, f.peak_freq_hz, f.peak_amp "
        "FROM raw_windows r JOIN fft_results f ON f.window_id = r.id "
        "JOIN window_labels l ON l.window_id = r.id "
        "WHERE l.label = ? ORDER BY r.id LIMIT 1",
        (label,),
    ).fetchone()
    if row is None:
        return None
    return {
        "sample_rate_hz": row[0],
        "ay": json.loads(row[1]),
        "freq_hz": json.loads(row[2]),
        "fft_ax": json.loads(row[3]),
        "fft_ay": json.loads(row[4]),
        "fft_az": json.loads(row[5]),
        "peak_axis": row[6],
        "peak_freq_hz": row[7],
        "peak_amp": row[8],
    }


def fetch_series_by_label(conn, label):
    """peak_freq_hz/peak_amp arrays (as parallel lists) for every window
    carrying `label`."""
    rows = conn.execute(
        "SELECT f.peak_freq_hz, f.peak_amp "
        "FROM raw_windows r JOIN fft_results f ON f.window_id = r.id "
        "JOIN window_labels l ON l.window_id = r.id "
        "WHERE l.label = ? ORDER BY r.id",
        (label,),
    ).fetchall()
    return [r[0] for r in rows], [r[1] for r in rows]
