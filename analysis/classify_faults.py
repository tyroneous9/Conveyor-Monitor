...
def compute_baseline(healthy_windows, band_center_hz, band_width_hz, baseline_fraction):
    ...
    values = np.array([band_amplitude(c["freq_hz"], c["fft_ay"], band_center_hz, band_width_hz) for c in fit_windows])
...
def main():
    ...
    for c in fit_windows:
        ...
        rows.append({"window_id": c["window_id"], "value": value, "true": "healthy",
